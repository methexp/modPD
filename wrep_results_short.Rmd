...
Next, we turn to the results of the TBS task, focusing on whether the absence of EC without memory is replicated with the simplified design.
...
We also report initial results from a new version of the procedure (i.e., 3ACE) that serves to illustrate how the TBS approach could be further developed by allowing for three (rather than two) retrieval experiences, and by assessing evaluations on a continuous rating scale (rather than as a binary choice).
