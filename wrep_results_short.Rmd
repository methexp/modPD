...
Importantly, proportions of TBS responses in line with US valence (*M =* `r mean(prop_correct_tbs_att$prop[prop_correct_tbs_att$correct_response=="correct"])`; *SD =* `r sd(prop_correct_tbs_att$prop[prop_correct_tbs_att$correct_response=="correct"])`) were not significantly different from the chance level, `r apa_print(t_prop_correct_a)$statistic`, *d =* `r d_prop_correct_a$Cohens_d`, and a Bayesian analysis yielded evidence for no effect, going beyond failing to find an EC effect for Attitude responses, $BF_{H(0, 15\%)}=$ `r bf_prop_tbs_att$BF` (Robustness Region: [`r bf_prop_tbs_att$RR$sd[1]`, `r ifelse(bf_prop_tbs_att$RR$sd[2]>1, "> 100%", bf_prop_tbs_att$RR$sd[2])`]). 
...
We did not find an EC effect for this category, `r apa_print(mod_noc_vma)$full_result`, and BF yielded evidence against it, $BF_{H(0, 23.5)}=$ `r bf_ev_no_conscious$BF`. 
...
Overall, valence memory was above chance (*M =* `r mean(prop_correct_3bs$prop[prop_correct_3bs$correct_response=="correct"])`; *SD =* `r sd(prop_correct_3bs$prop[prop_correct_3bs$correct_response=="correct"])`), `r apa_print(t_prop_3bs_o)$statistic`, *d =* `r d_prop_3bs_o$Cohens_d`, $BF_{H(0, 15\%)}=$ `r bf_prop_overall_3bs$BF`. 
...
The main results are displayed in Figure \@ref(fig:overall_3BS_figure). Importantly, we were interested in whether we find an EC effect when participants reported no memory (random guessing), as in the VMA task. 
The pattern of results was very similar to the one we found on the VMA task. 

Both frequentist and Bayesian analyses yielded evidence for an EC effect when participants made a memory attribution, `r apa_print(mod_memory_3bs)$full_result`, $BF_{H(0, 23.5)}=$ `r bf_ev_memory_3bs$BF`. 
When participants made a feeling/intuition attribution, the EC effect was not significant, `r apa_print(mod_intuition_3bs)$full_result`; however, BF was inconclusive, $BF_{H(0, 23.5)}=$ `r bf_ev_intuition_3bs$BF`. 
