...
First, we tested whether there was an EC effect overall, based on the final total sample (*N =* `r length(unique(dat$subject))`).
...
#visualize the data
apa_beeplot(data=dat, id="subject", dv="eval_change_score", factors=c( "eval_task","us_valence"), intercept =0, use = "all.obs", ylim=c(-50,50),
            xlab = "US Valence"
...
```{r overall_tbs_figure, fig.cap="Difference between evaluative change scores for CS+ and CS- (EC scores) as a function of Response button sets (TBS Task condition)", eval=FALSE}
...
# The critical test is whether an EC effect is found when participants lack memory (i.e., when they use the attitude buttons set). 
# The main results are displayed in Figure \@ref(fig:overall_tbs_figure). 
