...
<!-- In this section, we first analyzed (as a manipulation check) whether evaluative ratings were affected by the pairing manipulation. -->
<!-- Next, we describe the responses on the several memory tasks, and then test whether they were affected as predicted by the Pairing and Valence factors. -->
...
Figure \@ref(fig:ec2) illustrates pre-post evaluative changes as a function of Material, CS Valence, and US valence.
...
apa_beeplot(data=subset(d2), id="id", dv="Evaluative change", factors=c("CS Valence","US Valence","Material"), intercept=0, use = "all.obs", ylim=c(-6,6)) 
...
<!-- More detailed analyses are reported in the Appendix and will be summarized below. -->
...
```
<!-- The novel task asked participants to use the Memory buttons if they remember US valence, and the Attitude buttons if they do not. -->
...
<!-- ^[Note that absolute frequences for valenced foils are relatively small here due to the lower number of valenced as compared to neutral stimuli.] -->
<!-- ^[Material did not affect these Attitude responses.] -->

<!-- Taken together, the novel VM/A task has been successfully validated: 
Participants' metamemory judgments were able to separate paired from nonpaired stimuli; Memory judgments were largely accurate; and Attitude judgments reflected participants' attitudes.-->

<!-- We probed whether memory responses were systematically affected by participants' attitudes towards those stimuli. -->
