...
abstract: |
  For the last six decades, attitude research has capitalized on evaluative conditioning procedures to gain insight into how evaluative responses are acquired. In evaluative conditioning, a conditioned stimulus (CS; e.g., an unfamiliar soda brand) is paired with an unconditioned stimulus (US) of affective value (e.g., a pleasant picture). Following this pairing, a change in CS liking may be observed (e.g., the soda brand is liked better). A question with far-reaching theoretical and practical implications is whether the change in CS liking is found in the absence of conscious retrieval of the CS-US pairing. Here, we introduce a new conditional judgment procedure - the two-button-sets (TBS) task - for probing evaluative conditioning effects in the absence of feelings of remembering. In three experiments, the TBS is (1) is successfully validated; it is also used to (2) provide preliminary information on the "retrieval awareness" question, and (3) to examine an affect-consistent bias in memory judgments for CS-US pairings. Results do not support evaluative effects in the absence of feelings of remembering, and they oppose the view that affect-consistent bias is limited to memory uncertainty. We discuss these finding in light of previous evidence and of dual-learning models of attitudes. We also discuss limitations and research avenues related to the new procedure.
---
...
When relying on per-item procedures, researchers generally find no EC effect for CSs associated with incorrect memory reports in a separate memory task [@stahl_respective_2009]. 
...
If memory is absent for those CSs, then participants may use their pre-existing attitude to inform both their US memory judgment—leading to an incorrect valence-memory response—and their CS evaluations—leading to an evaluative effect opposite to the US’s valence (i.e., a reversed EC effect). 
...
In Process Dissociation studies, the issue does not apply anymore because participants are not forced to guess the US valence: if participants do not remember the US valence, they should report their attitude, not their memory response. 
...
We introduce here a new conditional judgment procedure: the two-button-sets (or TBS) task. 
The structure of the TBS is displayed in Figure \@ref(fig:task-displays) (right). 
For the sake of clarity, the Figure also illustrates the per-item and process dissociation task displays (left and center). 
...
On a given trial, only one judgment is reported, conditional upon memory retrieval:
...
This adaptation of the original process dissociation task allows memory and evaluation analyses at the item-within-person levels: 
...
In addition to this validation part, we used the TBS task to provide preliminary information on the retrieval awareness question (i.e., can evaluative conditioning effects be observed in the absence of conscious US Valence memory?) and whether attitudes contaminate memory measures (i.e., affect-as-information bias).
...
To sum up, the present study is primarily aimed at introducing the TBS and gathering empirical support for its validity for investigating how conscious retrieval of US valence relates to EC effects. 
In doing so, this research provides preliminary information about whether EC is found in the absence of conscious retrieval, and information relevant to the attitude-as-information bias. 
...
In line with Stahl et al. (2009), we found no EC in the absence of conscious US valence recollection.
...
Similar to Experiment 1, Experiment 2 also found no EC effect in the absence of conscious US valence retrieval on the TBS task: 
...
Experiment 3 examines whether EC is found in the absence of conscious US valence recollection, (1) with higher statistical power, and (2) by using a simplified design (i.e., a design that excludes nonpaired and valent stimuli, as typically done in EC studies). 
In doing so, this experiment provides what may be considered a more liberal (or even fairer) test for EC effects without feelings of US valence recollection. 
...
These results contrast with the TBS finding of no EC in the absence of feelings of US valence recollection, suggesting that a more fine-grained distinction of different mental states (i.e., three rather than two), and/or a more sensitive measurement (i.e., continuous rating scale rather than binary choice) may be more suited for detecting EC without memory.

To begin exploring possible reasons for such a discrepancy, we also included a third condition in which we adapted the TBS approach of conditionally assessing either memory or attitudes as a function of retrieval awareness.
...
The present evidence for the absence of EC in the absence of feelings of US valence recollection contrasts with the finding of EC for "random guessing" cases in Waroquier et al.'s (2020) Experiment 2 (both in the immediate and delayed test) as well as in a meta-analysis of their Experiments 1 and 2.
...
The present research introduced a new assessment procedure - the TBS task - for advancing the study of (potential) EC effects in the absence of retrieval awareness. 
In the empirical section, we validated this new task and we additionally used it to deliver preliminary insights into the influence of attitude-as-information biases typically found in retrieval-awareness studies, and insights relevant to the question of whether EC effects are found in the absence of feelings of US valence recollection. 
...
Experiment 3 further supported the robustness of conclusions derived from the TBS: similar conclusions about EC effects without conscious US valence retrieval were obtained when excluding (the otherwise informative) nonpaired and valent stimuli from the design. 
...
<!-- It is particularly important to highlight the TBS's usefulness for probing EC effects in the absence of retrieval awareness.  -->
<!-- When attitude-as-information bias operates, a reliable interpretation of memory reports is compromised. -->
<!-- However, we found here that pre-experimental CS valence -->
<!-- on TBS responses, it may be asked whether the findings of the absence of EC without memory can be interpreted. -->
...
<!-- Note, however, that attitude-as-information (i.e., pre-experimental CS valence)  -->
<!-- did not affect participants’ choice of the Memory vs. Attitude response sets.  -->
<!-- Thus, CS evaluations did not distort subjective decisions about recollecting US valence or not.  -->
<!-- For paired items, then, effects of US Valence on Memory-set responses may be interpreted as reflecting genuine US valence memory; and effects of US Valence on Attitude responses (if there were any) could be more confidently interpreted as reflecting EC effects in the absence of US valence recollection. -->

### Evaluative conditioning and retrieval awareness

A more secondary aim was to use the TBS to provide preliminary information about whether evaluative conditioning requires conscious retrieval of US valence.
Using the TBS task, we collected CS-level data on evaluations in the metacognitive absence of US memory. 
...
We actually found evidence for the absence of EC effects when participants' metacognitive judgments indicated memory for US valence to be absent.
...
The finding of no EC in the absence of retrieval awareness is more in line with previous conclusions obtained with per-item procedures that also found no EC when participants indicated not remembering the valence of the paired US [e.g., @stahl_respective_2009].
...
With a delay between judgments, their retrieval awareness measure met the immediacy criterion less well than the present TBS task.
One may, therefore, speculate that their results reflect the addition of noise to the evaluation-awareness relation, which could lead to an artificial "EC without US valence retrieval awareness" pattern.
...
### Going beyond the EC-without-retrieval-awareness issue
...
Beyond our primary validation goal, we used the TBS to obtain preliminary insights into EC effects without feelings of US valence recollection and into attitude-as-information effects. 
