---
title             : "Introducing a two-button-sets procedure for advancing the study of memory-independent evaluative conditioning effects"
...
They were then asked to attribute their response to one of three mental states (i.e., to indicate the basis of their valence-memory response): memory (i.e., remembered the pairing), feeling (familiarity or intuition), or random guessing.
In their Experiment 1, Waroquier et al. found EC effects not only for memory but also for feeling attributions; in their Experiment 2, EC was found for memory and random-guessing, but not for feeling attributions.
...
Participants completed either the TBS, VMA, or 3ACE task.
...
The results of these tasks are not reported in the present paper (which focuses on the validation of the TBS task); procedural details and results will be reported in an online appendix (osf.io/xxxxx).]
...
For exploratory purposes, we subsequently presented participants with another valence-memory task (VMA or TBS), followed by an US-identity memory task (see above).
...
Robust EC effects were found for memory attributions; the evidence for or against EC was inconclusive for "intuition" attributions; and EC was clearly absent for random-guessing/no-memory attributions.
...
These results support the validity of the results obtained with the TBS task.
In particular, the same results were obtained regardless of whether nonpaired and valent stimuli were included in the design or not; the findings from Experiments 1 and 2 are therefore not artifacts of their inclusion.
...
The present evidence for the absence of EC in the absence of memory contrasts with the finding of EC for "random guessing" cases in Waroquier et al.'s (2020) Experiment 2 (both on the immediate and delayed test) as well as in a meta-analysis of their Experiments 1 and 2.
Given that our study is a close replication of the studies by Waroquier, we can only speculate about the reasons for the discrepancy (i.e., there were differences in participant populations, US stimuli, language of instructions); more research is needed to try to replicate their EC effect for random-guessing attributions and to explain why Experiment 3 failed to do so.
With regard to the "intuition" attributions, our findings are more in line with those by Waroquier et al., who also found inconclusive evidence for these cases (i.e., there was evidence for feeling-based EC in Exp.1, but it was inconclusive in the immediate test of Exp.2, and there was evidence for the absence of EC on the delayed test).
...
We validated this new task in three experiments, which additional provided insights into the influence of attitude-as-information biases typically found in retrieval-awareness studies. 
...
Experiment 3 further confirmed that our central finding (of no EC without memory) was not an artifact of our including nonpaired and valent stimuli. 
...
Note, however, that attitude-as-information (i.e., pre-experimental CS valence) did not affect participants’ choice of the Memory vs. Attitude response sets. 
Thus, CS attitudes (pre-existing or learned) did not distort subjective decisions about having memory or not. 
...
In sum, we found evidence for the absence of EC when participants' metacognitive judgments indicated memory for US valence to be absent.
...
In the present Experiment 3, we failed to replicate this discrepancy. 
...
On the other hand, per-item measures of valence memory required participant's judgment knowledge (i.e., whether a CS had been paired with positive or negative USs), as well as their awareness of knowing (i.e., they ask participants to respond 'dont know' if they are unaware of any knowledge).
...
For both the by-item and TBS tasks, we assume that knowledge about one's subjective knowledge state (i.e., being aware of either having knowledge or of not having knowledge) can be used intentionally (i.e., participants intentionally select whether to report their memory or their attitude).
...
Here we have proposed and validated a novel approach for this kind of research, one that combines the advantages of the previous per-item and process-dissociation approaches and simultaneously assesses subjective memory states and evaluative effects; and we have illustrated how this new method can be further adapted and extended.
The present studies have also yielded novel findings (EC effects are absent when participants feel they have no knowledge about US valence; attitude-as-information effects when participants report remembering US valence) that we hope will stimulate future research on this topic.
