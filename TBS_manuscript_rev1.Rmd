...
In Experiment 1, random effects for interactions were also added; this was not possible in Experiment 2 because of the more complex design, which yielded a more complex model that was not supported given the relatively limited data. Omitting the interation terms from the random-effects structure of the model did not reduce the model's goodness of fit, $\chi^2(30)=9.69, p=.99$. 
...
The results of these tasks are not reported in the present paper (which focuses on the validation of the TBS task); procedural details as well as the results of these tasks are reported in an online appendix (osf.io/xxxxx).
...
Of the N participants who completed the study, x were excluded because they indicated that their data should not be used for scientific purposes, or that they did not pay attention.
An additional y participants were excluded because they failed the instruction checks.
We continued with data collection until we had at least n=60 in each condition of the valence-memory task factor.
...
Participants pre-rated 60 images, and the 40 most neutral ones were used as CSs.
Half of them were assigned to be paired with positive CSs, the other half with negative CSs.
Each CS was paired with a single US.
...
The procedure was similar to the previous studies (with procedural details closely following those from Waroquier et al.).
After collecting pre-study ratings and performing individual CS-US assignments, the learning phase was presented simultaneously on the screen for x s (with a y ms ISI).
...
After the study phase, half of participants first gave post-study ratings of all 40 CSs and then engaged in the valence-memory task (i.e., TBS, Waroquier, or 3BS); the other half completed the tasks in the reverse order.
...
For exploratory reasons, we subsequently presented participants with another valence-memory task, followed by an US-identity memory task (see above).
Finally, participants were asked whether they paid attention during the learning phase, whether they thought their data should be analyzed for scientific purposes (or were subpar for any reason), and about the setting in which they completed the study (e.g., in the lab, at home, etc.).
...
In other words, whereas EC may require pairing memory, it may not require retrieval of those memories at the time of CS evaluations.
