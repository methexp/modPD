...
colnames(n_attrib_tbs1) = c("subject", "attrib_buttons", "n_attrib_tbs")

n_attrib_tbs1$n_attrib_tbs = as.numeric(n_attrib_tbs1$n_attrib_tbs)

m_prop_Memory1 = mean(n_attrib_tbs1$n_attrib_tbs[n_attrib_tbs1$attrib_buttons=="Memory"])
sd_prop_Memory1 = sd(n_attrib_tbs1$n_attrib_tbs[n_attrib_tbs1$attrib_buttons=="Memory"])

m_prop_Attitude1 = mean(n_attrib_tbs1$n_attrib_tbs[n_attrib_tbs1$attrib_buttons=="Attitude"])
sd_prop_Attitude1 = sd(n_attrib_tbs1$n_attrib_tbs[n_attrib_tbs1$attrib_buttons=="Attitude"])

describeBy(n_attrib_tbs1$n_attrib_tbs, n_attrib_tbs1$attrib_buttons)
```
...
colnames(n_attrib_tbs2) = c("subject", "attrib_buttons", "n_attrib_tbs")

n_attrib_tbs2$n_attrib_tbs = as.numeric(n_attrib_tbs2$n_attrib_tbs)

m_prop_Memory2 = mean(n_attrib_tbs2$n_attrib_tbs[n_attrib_tbs2$attrib_buttons=="Memory"])
sd_prop_Memory2 = sd(n_attrib_tbs2$n_attrib_tbs[n_attrib_tbs2$attrib_buttons=="Memory"])

m_prop_Attitude2 = mean(n_attrib_tbs2$n_attrib_tbs[n_attrib_tbs2$attrib_buttons=="Attitude"])
sd_prop_Attitude2 = sd(n_attrib_tbs2$n_attrib_tbs[n_attrib_tbs2$attrib_buttons=="Attitude"])

describeBy(n_attrib_tbs2$n_attrib_tbs, n_attrib_tbs2$attrib_buttons)
```
...
colnames(n_attrib_tbs3) = c("subject", "attrib_buttons", "n_attrib_tbs")

n_attrib_tbs3$n_attrib_tbs = as.numeric(n_attrib_tbs3$n_attrib_tbs)

m_prop_Memory3 = mean(n_attrib_tbs3$n_attrib_tbs[n_attrib_tbs3$attrib_buttons=="memory"])
sd_prop_Memory3 = sd(n_attrib_tbs3$n_attrib_tbs[n_attrib_tbs3$attrib_buttons=="memory"])

m_prop_Attitude3 = mean(n_attrib_tbs3$n_attrib_tbs[n_attrib_tbs3$attrib_buttons=="attitude"])
sd_prop_Attitude3 = sd(n_attrib_tbs3$n_attrib_tbs[n_attrib_tbs3$attrib_buttons=="attitude"])

describeBy(n_attrib_tbs3$n_attrib_tbs, n_attrib_tbs3$attrib_buttons)
```
...
colnames(n_attrib_vma) = c("subject", "attrib_buttons", "n_attrib_vma")

n_attrib_vma$n_attrib_vma = as.numeric(n_attrib_vma$n_attrib_vma)

m_prop_Memory_vma = mean(n_attrib_vma$n_attrib_vma[n_attrib_vma$attrib_buttons=="memory_attrib_button"])
sd_prop_Memory_vma = sd(n_attrib_vma$n_attrib_vma[n_attrib_vma$attrib_buttons=="memory_attrib_button"])

m_prop_intuition_vma = mean(n_attrib_vma$n_attrib_vma[n_attrib_vma$attrib_buttons=="intuition_feeling_button"])
sd_prop_intuition_vma = sd(n_attrib_vma$n_attrib_vma[n_attrib_vma$attrib_buttons=="intuition_feeling_button"])

m_prop_guess_vma = mean(n_attrib_vma$n_attrib_vma[n_attrib_vma$attrib_buttons=="guess_attrib_button"])
sd_prop_guess_vma = sd(n_attrib_vma$n_attrib_vma[n_attrib_vma$attrib_buttons=="guess_attrib_button"])
```
...
colnames(n_attrib_3bs) = c("subject", "response_set", "n_attrib")

n_attrib_3bs$n_attrib = as.numeric(n_attrib_3bs$n_attrib)

m_prop_Memory_3ace = mean(n_attrib_3bs$n_attrib[n_attrib_3bs$response_set=="memory_button"])
sd_prop_Memory_3ace = sd(n_attrib_3bs$n_attrib[n_attrib_3bs$response_set=="memory_button"])

m_prop_intuition_3ace = mean(n_attrib_3bs$n_attrib[n_attrib_3bs$response_set=="intuition_scale"])
sd_prop_intuition_3ace = sd(n_attrib_3bs$n_attrib[n_attrib_3bs$response_set=="intuition_scale"])

m_prop_guess_3ace = mean(n_attrib_3bs$n_attrib[n_attrib_3bs$response_set=="guess_scale"])
sd_prop_guess_3ace = sd(n_attrib_3bs$n_attrib[n_attrib_3bs$response_set=="guess_scale"])
```
...
kbl(table_appendix_tbs, col.names = gsub("[.]", " ", names(table_appendix_tbs)), booktabs=TRUE, align="c", caption="Proportions of Memory and Attitude-set responses and of valence memory accuracy (with tests against the chance level) in the Two-buttons-sets procedure of each experiment.", label="a1_tbs") %>%
  add_footnote("Note. In Experiments 1 and 2, the analyses were performed only on the responses to the paired neutral conditioned stimuli. 't test': two-tailed one-sample t-tests on the proportions of correct valence responses against the chance level (mu=.5). 'Cohen d 95 CI': 95% Confidence Intervals of the effect sizes indexed by Cohen's d. BF: Bayes Factors -- we modeled the distribution of true effect sizes under H1 as a half-normal distribution with an SD of 15 percent (see main text). 'Robustness Region': range of SDs of the H1 distribution that support the same conclusion as the reported BF (in the analyses, max SD = 2).", notation="none") %>%
...
kbl(table_vma_3ace, col.names = gsub("[.]", " ", names(table_vma_3ace)), booktabs=TRUE, align="c", caption="Proportions (and SD) of attributions and of valence memory accuracy (with tests against the chance level) in the Valence Memory with Attributions (VMA) and the Three-attribution, continuous evaluation (3ACE) tasks (Experiment 3).", label="a2_vma_3ace") %>%
add_footnote("Note. 't test': two-tailed one-sample t-tests on the proportions of correct valence responses against the chance level (mu=.5). 'Cohen d 95 CI': 95% Confidence Intervals of the effect sizes indexed by Cohen's d. BF: Bayes Factors -- we modeled the distribution of true effect sizes under H1 as a half-normal distribution with an SD of 15 percent (see main text). 'Robustness Region': range of SDs of the H1 distribution that support the same conclusion as the reported BF (in the analyses, max SD = 2). VMA: Valence Memory with Attributions task; 3ACE: Three-Attribution, Continuous Evaluation task.", notation="none") %>%
  kable_styling(latex_options = c("scale_down")) %>%
  collapse_rows(columns = 1, latex_hline = "major", valign = "middle")
```